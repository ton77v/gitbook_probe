...
This security fix addresses a Denial of Service (DoS) vulnerability in the Axios HTTP client library used by Kibana. The vulnerability allowed attackers to bypass memory limits by using large data: URIs, causing applications to consume unbounded memory and crash.
</details>
<details>
...
These security fixes address two vulnerabilities in the Netty framework used by Elasticsearch. CVE-2025-58057 fixes a DoS vulnerability that could cause out-of-memory errors, while CVE-2025-58056 resolves an HTTP request smuggling vulnerability that could allow attackers to bypass security controls.
</details>
<details>
...
This feature allows administrators to customize audit log event formats directly through configuration without writing custom code, providing flexibility in how audit events are formatted for different logging and monitoring requirements.
</details>
<details>
...
Two new predefined audit serializers are now available. One captures all audit events, while the other includes the full request query in addition to all events, providing more comprehensive logging options for security monitoring.
</details>
<details>
...
These new rules provide more granular control over Kibana authentication and authorization processes, offering greater flexibility in security configuration compared to the combined ror_kbn_auth rule.
</details>
<details>
...
This enhancement adds a clock skew tolerance configuration option for OIDC authentication, improving reliability in environments with time synchronization challenges.
</details>
<details>
...
This enhancement provides administrators with control over Kibana's behavior when encountering Elasticsearch high watermark errors, allowing continued operation during temporary storage capacity issues.
</details>
<details>
...
This fix resolves a security issue where user sessions were not properly invalidated during logout when using the ror_kbn_auth rule with local group configurations, preventing potential unauthorized access after logout.
</details>
<details>
...
This fix restores the functionality of keyword field value suggestions in Kibana's Discover and Data View filter interfaces, improving the data exploration experience.
</details>
<details>
...
This fix addresses an issue where integration-based application options remained visible in search results despite being configured as hidden.
</details>
<details>
...
This fix resolves a visibility issue where the Index Management application remained visible in search results when configured as hidden.
</details>
<details>
<summary><strong>🐞 Fix</strong> (KBN) Resolved an issue with CSRF token override when multiple browser tabs were open</summary>
This fix addresses a Cross-Site Request Forgery (CSRF) token conflict that occurred when users had multiple Kibana browser tabs open simultaneously.
</details>
<details>
<summary><strong>🐞 Fix</strong> (KBN) Fixed OIDC compatibility for Kibana 7.10.2 and earlier</summary>
This fix restores OIDC authentication compatibility for older Kibana versions 7.10.2 and earlier.
</details>
<details>
...
This fix restores backward compatibility for custom audit log serializers that extend the DefaultAuditLogSerializer class, though custom serializers compiled against ROR 1.65 or 1.66 need recompilation.
</details>
<details>
<summary><strong>🐞 Fix</strong> (ES) Fixed a defect that broke the "Snapshot and Restore" functionality in Kibana</summary>
This fix resolves an issue that prevented the Snapshot and Restore functionality from working properly in Kibana, allowing users to successfully create, manage, and restore Elasticsearch snapshots.
</details>
